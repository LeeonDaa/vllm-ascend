...
class TestEnvVariables(TestBase):
        ...
        for var_name in self.env_vars:
            with self.subTest(var=var_name):
                ...
                try:
                    ...
                    if var_name in ("VLLM_ASCEND_KVPOOL_RANGE_DEBUG", "VLLM_ASCEND_KVPOOL_LAYER_DIAG"):
                        test_vals = ["0", "1"]
    ...
    def test_kvpool_layer_diag_is_strict_and_disabled_by_default(self):
        name = "VLLM_ASCEND_KVPOOL_LAYER_DIAG"
        original_val = os.environ.pop(name, None)
        try:
            self.assertFalse(getattr(envs_ascend, name))
            for value, expected in (("0", False), ("1", True)):
