...
class KVPoolScheduler:
    ...
    def _lookup_layerwise_with_coordinator(
        ...
        def query_group_hits(
            ...
            if self.block_key_hybrid:
                states = []
                # A block owns one key per (PP stage, DCP shard, saving head), so
                # the batch must scale with all three to keep batches block-aligned.
                batch_size = (
                    self.layerwise_max_transfer_blocks * (self.tp_size // self.put_step) * self.pp_size * self.dcp_size
                    or len(all_keys)
                )
