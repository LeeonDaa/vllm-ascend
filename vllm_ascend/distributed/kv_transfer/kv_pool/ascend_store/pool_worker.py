...
class KVPoolWorker:
    ...
    def process_layer_data(self, requests: list[ReqMeta]) -> None:
        ...
        for physical_layer in range(self.num_layers):
            for group_id, layer_idx_in_group in self._local_group_layers(physical_layer):
    ...
    def _local_group_layers(self, physical_layer: int) -> list[tuple[int, int]]:
        """Group entries of a physical layer this rank actually stores.
        ...
        return [
            (group_id, self.group_layer_local_index.get(group_id, {}).get(physical_layer, layer_idx_in_group))
