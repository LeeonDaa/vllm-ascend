...
class KVPoolWorker:
    ...
    def _build_group_layer_builders(self) -> list[LayerBatchBuilder]:
        builders = []
        # Mooncake's count follows the local physical cache layers, including
        # draft layers discovered after the base model's PP partition was set.
        # Keep the existing single-group key-plane behavior for other backends.
        if self.num_kv_cache_groups == 1 or self.use_block_key_layerwise:
            self.layerwise_key_layers = self.num_layers
        for group_id in range(self.num_kv_cache_groups):
