...
def create_kvpp_cache_allocation_plan(
    ...
    unsupported = sorted(
        name for name, spec in logical_spec.items() if isinstance(spec, AscendIndexerKPoolStateSpec)
    )
    if unsupported:
        # The state cache is request-owned state, not a block cache that can
        # be replaced by another rank's copy.
        raise ValueError("KVPP does not support request-owned indexer state caches: " + ", ".join(unsupported[:3]))
